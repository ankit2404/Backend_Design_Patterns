...
public class OrderOneWinningStrategy implements WinningStrategy {
    ...
    private List<Map<Symbol, Integer>> rows = new ArrayList<>();
    private List<Map<Symbol, Integer>> cols = new ArrayList<>();
    ...
    public boolean checkWinner(Move move, Board board) {
        ...
        Map<Symbol, Integer> rowMap = rows.get(r);
        rowMap.put(player.getSymbol(), rowMap.getOrDefault(player.getSymbol(), 0) + 1);

        Map<Symbol, Integer> colMap = cols.get(c);
